...
def check_aliens_bottom(ai_settings, stats, sb, screen, ship, aliens, bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            #Происходит по же, что и при столкновении с кораблем
...
def check_play_button(ai_settings, screen, stats, sb, play_button, ship, aliens,
    ...
    if button_clicked and not stats.game_active:
        ...
        # Music play
        pygame.mixer.music.load('sound/music/Level1.wav')
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    #Создание пришельца и вычисление колицества пришельцев в ряду
    alien = Alien(ai_settings, screen)
...
def fire_bullet(ai_settings, screen, ship, bullets):
    ...
    if len(bullets) < ai_settings.bullets_allowed:
        # Play sound
        laser = pygame.mixer.Sound('sound/effects/sfx_wpn_laser7.wav')
        laser.play()
